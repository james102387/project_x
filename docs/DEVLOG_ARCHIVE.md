...
## 2026-04-13 — UI Overhaul

### What changed
- **Persistent KG banner** — All tabs now show which knowledge graph is active (name, triplet count, entity count) in a colored banner at the top. Banner updates when KG is switched.
- **Merged KG Explorer into Knowledge Graph tab** — Eliminated a separate tab. Entity search, predicate summary, and entity list now live under "Explore" section within the KG tab.
- **Clear section hierarchy in every tab** — Used `gr.Group()` with numbered section headers (1. Extract, 2. Review Triplets, 3. Generate Q&A, 4. Test) to separate functional blocks with visual borders.
- **Scrollable facts table** — KG facts now displayed as a `gr.Dataframe` with `height=400` instead of a markdown table. Supports scrolling, column resizing.
- **Text wrapping in all tables** — Added `wrap=True` to all `gr.Dataframe` instances so questions/answers are fully readable instead of cut off.
- **Removed redundant/confusing buttons** — Removed "Save & Accept Pending" (was duplicate of "Accept All Pending"). Removed "Reset to Remulak" (use KG selector instead). Removed "Refresh Dashboard" (dashboard updates on batch select).
- **Clarified Ingest Documents labels** — "Auto-Accepted" moved to a collapsed accordion with explanation ("above confidence threshold"). "Pending Review" section explains "below threshold, needs your approval". Stats text explains what each category means.
- **Improved KG tab import section** — Upload/paste section relabeled "Import Structured Data" with note directing users to Ingest Documents for extraction. Distinguishes pre-formatted KG import from document extraction.
- **Review tab restructured** — Dashboard shows concise pending counts instead of item-level list. Batch overview table moved above question review for context. "Detector Known Gaps" renamed to "Known Detection Failures" with explanation. All sections use `gr.Group()` for visual separation.
- **Review dashboard simplified** — `format_review_dashboard()` in `review.py` now shows a concise summary (pending count, accepted/rejected counts, batch count) instead of verbose per-file listings.

### Decisions
- Merged KG Explorer into KG tab rather than keeping as separate tab — reduces tab count and the explorer was just entity search + predicate summary, which belongs with KG management.
- Facts displayed as DataFrame instead of markdown — markdown tables can't scroll and are unreadable at 2000+ rows.
- Removed "Refresh Dashboard" button — the dashboard refreshes naturally when switching batches. Users can reload the page for a full refresh.
...
- **KG audit tool** — `src/crystal/tools/kg/audit.py`: Fast, deterministic health check (zero LLM cost). Runs validation gates on all KG facts.
- **KG proofreader** — `src/crystal/tools/kg/proofreader.py`: Two-pass deep clean. Pass 1: fast gates (free). Pass 2: LLM proofreading. Tiered trust: hard failures auto-delete, soft+LLM → auto-delete, LLM-only → human review queue.
- **KG cleaned** — Proofreader fast-mode deleted 764 garbage facts. Backfilled 306 empty `source` fields. Health score: 0.75 → 0.93.
- **59 new tests** — 1,122 tests total.

### Decisions
- Tiered trust policy: fast gates deterministic → auto-delete. LLM rejections → human review queue. Exception: soft gate + LLM rejection = auto-delete.
- `normalize_predicate()` substring match removed entirely. Better to reject unmapped predicates.
