...
def _build_grounded_prompt(question: str) -> tuple[str | None, str]:
    """Run Crystal's KG pipeline on a question.

    Returns (grounded_prompt_or_None, prompt_type).
    If KG detects and augments, returns the grounded prompt.
    If KG answers directly, returns the formatted facts (no LLM needed).
...
def run_reasoning_benchmark(
    ...
    for i, case in enumerate(cases):
        ...
        grounded_prompt, prompt_type = _build_grounded_prompt(question)
...
def main():
    ...
    print(f"\n--- Reasoning Cost Benchmark (model: {args.model}) ---")
    comparisons = run_reasoning_benchmark(
        BENCHMARK_CASES, args.model, filter_type=args.cases,
    )
